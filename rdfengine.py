...
OPERATORS = {">": operator.gt, "<=": operator.le, "==": operator.eq}
...
def evaluate_sparql_condition(engine_graph, flow_uri, instance_uri):
    ...
    result = engine_graph.query(
        str(query_string), initBindings={"instance": URIRef(instance_uri)}
    )
...
# 2. The Registry
TOPIC_REGISTRY = {"calculate_tax": tax_calculator}
...
def handle_token_arrival(engine_graph, token_uri, gateway_uri, instance_uri):
    ...
    is_joined = engine_graph.query(
        JOIN_QUERY, initBindings={"gateway": gateway_uri, "instance": instance_uri}
    ).askAnswer
